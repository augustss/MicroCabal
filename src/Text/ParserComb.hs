...
longest :: LastFail t -> LastFail t -> LastFail t
...
longests :: [LastFail t] -> LastFail t
...
firstToken :: TokenMachine tm t => tm -> [t]
firstToken tm =
...
runP :: Prsr tm t a -> (tm -> Res tm t a)
runP (P p) = p

mapTokenState :: (tm -> tm) -> Prsr tm t ()
...
-- Left biased choice
infixl 3 <|<
(<|<) :: Prsr tm t a -> Prsr tm t a -> Prsr tm t a
(<|<) p q = P $ \ t ->
  case runP p t of
    Many [] lfa ->
      case runP q t of
...
satisfy :: TokenMachine tm t => String -> (t -> Bool) -> Prsr tm t t
satisfy msg f = P $ \ acs ->
  case tmNextToken acs of
    r@(c, _) | f c -> Many [r] noFail
    _ -> Many [] (LastFail (tmLeft acs) (firstToken acs) [msg])

satisfyM :: TokenMachine tm t => String -> (t -> Maybe a) -> Prsr tm t a
satisfyM msg f = P $ \ acs ->
  case tmNextToken acs of
    (c, cs) ->
      case f c of
        Just a  -> Many [(a, cs)] noFail
        Nothing -> Many [] (LastFail (tmLeft acs) (firstToken acs) [msg])

satisfyMany :: TokenMachine tm t => (t -> Bool) -> Prsr tm t [t]
...
infixl 9 <?>
(<?>) :: Prsr tm t a -> String -> Prsr tm t a
(<?>) p e = P $ \ t ->
--  trace ("<?> " ++ show e) $
  case runP p t of
...
nextToken :: TokenMachine tm t => Prsr tm t t
nextToken = P $ \ cs ->
...
emany :: Prsr tm t a -> Prsr tm t [a]
emany p = esome p <|< pure []

esome :: Prsr tm t a -> Prsr tm t [a]
esome p = (:) <$> p <*> emany p

eoptional :: Prsr tm t a -> Prsr tm t (Maybe a)
eoptional p = (Just <$> p) <|< pure Nothing

choice :: TokenMachine tm t => [Prsr tm t a] -> Prsr tm t a
choice [] = empty
choice ps = foldr1 (<|>) ps
...
esepBy1 :: Prsr tm t a -> Prsr tm t sep -> Prsr tm t [a]
esepBy1 p sep = (:) <$> p <*> emany (sep *> p)

esepBy :: Prsr tm t a -> Prsr tm t sep -> Prsr tm t [a]
esepBy p sep = esepBy1 p sep <|< pure []

esepEndBy :: Prsr tm t a -> Prsr tm t sep -> Prsr tm t [a]
